...
public class VendingMachineLargeBlock extends TraderBlockTallWideRotatable implements IItemTraderBlock {
    ...
    public List<Vec3f> GetStackRenderPos(int tradeSlot, BlockState state, boolean isDoubleTrade) {
        ...
        if(tradeSlot == 0)
        {
            Vec3f rightOffset = MathUtil.VectorMult(right, 3.5f/16f);
            Vec3f vertOffset = MathUtil.VectorMult(up, 27f/16f);
            firstPosition = MathUtil.VectorAdd(offset, forwardOffset, rightOffset, vertOffset);
        }
        else if(tradeSlot == 1)
        {
            Vec3f rightOffset = MathUtil.VectorMult(right, 10.5f/16f);
            Vec3f vertOffset = MathUtil.VectorMult(up, 27f/16f);
            firstPosition =  MathUtil.VectorAdd(offset, forwardOffset, rightOffset, vertOffset);
        }
        else if(tradeSlot == 2)
        {
            Vec3f rightOffset = MathUtil.VectorMult(right, 17.5f/16f);
            Vec3f vertOffset = MathUtil.VectorMult(up, 27f/16f);
            firstPosition =  MathUtil.VectorAdd(offset, forwardOffset, rightOffset, vertOffset);
        }
        else if(tradeSlot == 3)
        {
            Vec3f rightOffset = MathUtil.VectorMult(right, 24.5f/16f);
            Vec3f vertOffset = MathUtil.VectorMult(up, 27f/16f);
            firstPosition =  MathUtil.VectorAdd(offset, forwardOffset, rightOffset, vertOffset);
        }
        else if(tradeSlot == 4)
        {
            Vec3f rightOffset = MathUtil.VectorMult(right, 3.5f/16f);
            Vec3f vertOffset = MathUtil.VectorMult(up, 20f/16f);
            firstPosition =  MathUtil.VectorAdd(offset, forwardOffset, rightOffset, vertOffset);
        }
        else if(tradeSlot == 5)
        {
        ...
        {
            Vec3f rightOffset = MathUtil.VectorMult(right, 3.5f/16f);
            Vec3f vertOffset = MathUtil.VectorMult(up, 13f/16f);
            firstPosition =  MathUtil.VectorAdd(offset, forwardOffset, rightOffset, vertOffset);
        }
        else if(tradeSlot == 9)
        {
