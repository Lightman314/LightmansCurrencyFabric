		...
		if(coinData != null)
		{
			switch (LCConfig.SERVER.coinTooltipType.get()) {
				...
				case VALUE -> {
					...
					tooltip.add(Text.translatable("tooltip.lightmanscurrency.coinworth.value", LCConfig.formatValueDisplay(value)).formatted(Formatting.YELLOW));
					if (stack.getCount() > 1)
						tooltip.add(Text.translatable("tooltip.lightmanscurrency.coinworth.value.stack", LCConfig.formatValueDisplay(value * stack.getCount())).formatted(Formatting.YELLOW));
				}
				default -> {
				} //Default is NONE
			}
