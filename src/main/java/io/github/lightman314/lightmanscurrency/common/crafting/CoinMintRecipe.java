...
public class CoinMintRecipe implements Recipe<Inventory> {
    ...
    @Override
    public ItemStack craft(Inventory inventory, DynamicRegistryManager manager) { return this.getResult(); }

    @Override
    public boolean fits(int width, int height) { return true; }

    @Override
    public ItemStack getOutput(DynamicRegistryManager manager) { return this.getResult(); }
    ...
    public static class Serializer implements RecipeSerializer<CoinMintRecipe> {
        ...
        public CoinMintRecipe read(Identifier id, JsonObject json) {
            ...
            ItemStack result = new ItemStack(Registries.ITEM.get(new Identifier(json.get("result").getAsString())));
            if(result.isEmpty())
            {
