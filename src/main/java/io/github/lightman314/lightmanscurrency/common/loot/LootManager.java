...
public class LootManager {
    ...
    public static boolean ConfigContainsEntity(@NotNull StringListOption configOption, @NotNull EntityType<?> entityType)
    {
        ...
        AtomicReference<Stream<TagKey<EntityType<?>>>> temp = new AtomicReference<>(null);
        Stream<TagKey<EntityType<?>>> entityTags;
        Registry.ENTITY_TYPE.getEntry(RegistryKey.of(Registry.ENTITY_TYPE.getKey(), Registry.ENTITY_TYPE.getId(entityType))).ifPresent(
                e -> temp.set(e.streamTags())
        );
        entityTags = temp.get() == null ? Stream.empty() : temp.get();
