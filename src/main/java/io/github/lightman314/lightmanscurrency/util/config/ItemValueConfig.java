...
public class ItemValueConfig implements Supplier<Item> {
    ...
    public Item get() {
        ...
        try{ this.cachedItem = Registry.ITEM.get(new Identifier(itemID));
    ...
    private static Supplier<Item> convertDefault(Identifier defaultItem) { return () -> Registry.ITEM.get(defaultItem); }

    private static boolean IsValidInput(Object o) {
        if(o instanceof String s)
        {
            try{
                return Registry.ITEM.get(new Identifier(s)) != Items.AIR;
