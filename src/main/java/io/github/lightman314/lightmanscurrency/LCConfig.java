...
public class LCConfig {
    ...
    {
        return LCConfig.SERVER.valueFormat.get().replace("{value}", formatValueOnly(value));
    }
    ...
    public static void reloadVillagerOverrides() {
        ...
        for(int i = 0; i < overrides.size(); ++i)
        {
            try {
                ...
                try {
                    villagerType = new Identifier(split[0]);
                ...
                if(item == Items.AIR)
                    throw new RuntimeException("Item '" + itemType + "' is not a registered item, or is air!");
    ...
    public static Item getEmeraldReplacementItem(String trader) {
        ...
        return LCConfig.COMMON.defaultTraderCoin.get();
    ...
    public static class Client
    {
        ...
        private static final Supplier<ForgeConfigSpec> SPEC_SUPPLIER = () -> LCConfig.clientSpec;
    ...
    public static class Common
    {
        ...
        private static final Supplier<ForgeConfigSpec> SPEC_SUPPLIER = () -> LCConfig.commonSpec;
        ...
        Common(ForgeConfigSpec.Builder builder)
        {
            ...
            this.defaultTraderCoin = ItemValueConfig.define(builder
                    ...
                    ,"defaultTraderCoin", new Identifier("lightmanscurrency","coin_emerald"),
                    SPEC_SUPPLIER);
            ...
            this.entityDropsT1 = builder
                    ...
                    .defineList("entityListT1", LootManager.ENTITY_COPPER_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            //Iron
            this.entityDropsT2 = builder
                    .comment("Entities that will occasionally drop T1 -> T2 loot.")
                    .defineList("entityListT2", LootManager.ENTITY_IRON_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            //Gold
            this.entityDropsT3 = builder
                    .comment("Entities that will occasionally drop T1 -> T3 loot.")
                    .defineList("entityListT3", LootManager.ENTITY_GOLD_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            //Emerald
            this.entityDropsT4 = builder
                    .comment("Entities that will occasionally drop T1 -> T4 loot.")
                    .defineList("entityListT4", LootManager.ENTITY_EMERALD_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            //Diamond
            this.entityDropsT5 = builder
                    .comment("Entities that will occasionally drop T1 -> T5 loot.")
                    .defineList("entityListT5", LootManager.ENTITY_DIAMOND_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            //Netherite
            this.entityDropsT6 = builder
                    .comment("Entities that will occasionally drop T1 -> T6 loot.")
                    .defineList("entityListT6", LootManager.ENTITY_NETHERITE_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);

            //Boss
            this.bossEntityDropsT1 = builder
                    .comment("Entities that will drop a large amount of T1 loot.")
                    .defineList("bossEntityListT1", LootManager.ENTITY_BOSS_COPPER_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.bossEntityDropsT2 = builder
                    .comment("Entities that will drop a large amount of T1 -> T2 loot.")
                    .defineList("bossEntityListT2", LootManager.ENTITY_BOSS_IRON_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.bossEntityDropsT3 = builder
                    .comment("Entities that will drop a large amount of T1 -> T3 loot.")
                    .defineList("bossEntityListT3", LootManager.ENTITY_BOSS_GOLD_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.bossEntityDropsT4 = builder
                    .comment("Entities that will drop a large amount of T1 -> T4 loot.")
                    .defineList("bossEntityListT4", LootManager.ENTITY_BOSS_EMERALD_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.bossEntityDropsT5 = builder
                    .comment("Entities that will drop a large amount of T1 -> T5 loot.")
                    .defineList("bossEntityListT5", LootManager.ENTITY_BOSS_DIAMOND_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.bossEntityDropsT6 = builder
                    .comment("Entities that will drop a large amount of T1 -> T6 loot.")
                    .defineList("bossEntityListT6", LootManager.ENTITY_BOSS_NETHERITE_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            ...
            this.chestDropsT1 = builder
                    ...
                    .defineList("chestListT1", LootManager.CHEST_COPPER_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.chestDropsT2 = builder
                    .comment("Chests that will occasionally spawn T1 -> T2 loot.")
                    .defineList("chestListT2", LootManager.CHEST_IRON_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.chestDropsT3 = builder
                    .comment("Chests that will occasionally spawn T1 -> T3 loot.")
                    .defineList("chestListT3", LootManager.CHEST_GOLD_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.chestDropsT4 = builder
                    .comment("Chests that will occasionally spawn T1 -> T4 loot.")
                    .defineList("chestListT4", LootManager.CHEST_EMERALD_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.chestDropsT5 = builder
                    .comment("Chests that will occasionally spawn T1 -> T5 loot.")
                    .defineList("chestListT5", LootManager.CHEST_DIAMOND_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
            this.chestDropsT6 = builder
                    .comment("Chests that will occasionally spawn T1 -> T6 loot.")
                    .defineList("chestListT6", LootManager.CHEST_NETHERITE_DROPLIST.stream().map(Identifier::toString).toList(), o -> o instanceof String);
    ...
    public static class Server
    {
        ...
        private static final Supplier<ForgeConfigSpec> SPEC_SUPPLIER = () -> LCConfig.serverSpec;
        ...
        Server(ForgeConfigSpec.Builder builder)
        {
            ...
            this.valueBaseCoin = ItemValueConfig.define(builder
                    ...
                    ,"baseValueCoin", new Identifier("lightmanscurrency","coin_copper"), MoneyUtil::isVisibleCoin,
                    SPEC_SUPPLIER);
    ...
    public static final LCConfig.Server SERVER;

    static
    {
        //Client
        final Pair<Client,ForgeConfigSpec> clientPair = new ForgeConfigSpec.Builder().configure(LCConfig.Client::new);
        clientSpec = clientPair.getRight();
        CLIENT = clientPair.getLeft();
        //Common
        final Pair<Common,ForgeConfigSpec> commonPair = new ForgeConfigSpec.Builder().configure(LCConfig.Common::new);
        commonSpec = commonPair.getRight();
        COMMON = commonPair.getLeft();
        //Server
        final Pair<Server,ForgeConfigSpec> serverPair = new ForgeConfigSpec.Builder().configure(LCConfig.Server::new);
