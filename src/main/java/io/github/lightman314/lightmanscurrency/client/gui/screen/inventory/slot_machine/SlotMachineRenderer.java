...
public final class SlotMachineRenderer {
    ...
    private void startAnimation()
    {
        ...
        while(lockDelay.size() < randomLines.size())
            lockDelay.add(rand.nextBetween(GetAnimationTime() - 11, GetAnimationTime() - 1));
    ...
    public void render(DrawContext gui, float partialTicks)
    {
        ...
        for(int i = 0; i < SlotMachineEntry.ITEM_LIMIT; ++i)
        {
            this.lines.get(i).render(gui, partialTicks, startX, y);
        ...
        gui.getMatrices().translate(0d,0d,250d);
        gui.drawTexture(GUI_TEXTURE, this.screen.getGuiLeft(), this.screen.getGuiTop(), 0, 0, this.screen.getImageWidth(), this.screen.getImageHeight());
        gui.getMatrices().pop();
