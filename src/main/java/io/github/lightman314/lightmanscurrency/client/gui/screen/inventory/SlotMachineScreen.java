...
public class SlotMachineScreen extends MenuScreen<SlotMachineMenu> implements ScrollListener.IScrollListener, ScrollBarWidget.IScrollable {
    ...
    protected void init() {
        ...
        this.buttonOpenStorage = this.addDrawableChild(IconAndButtonUtil.storageButton(this.getGuiLeft() + TraderMenu.SLOT_OFFSET - 20, this.getGuiTop() + 118, this::OpenStorage, () -> this.handler.getTrader() != null && this.handler.getTrader().hasPermission(this.handler.player, Permissions.OPEN_STORAGE)));

        this.buttonCollectCoins = this.addDrawableChild(IconAndButtonUtil.collectCoinButton(this.getGuiLeft() + TraderMenu.SLOT_OFFSET - 20, this.getGuiTop() + 138, this::CollectCoins, this.handler.player, this.handler::getTrader));

        this.buttonOpenTerminal = this.addDrawableChild(IconAndButtonUtil.backToTerminalButton(this.getGuiLeft() + TraderMenu.SLOT_OFFSET - 20, this.getGuiTop() + this.getImageHeight() - 20, this::OpenTerminal, this::showTerminalButton));

        LazyWidgetPositioner.create(this, LazyWidgetPositioner.MODE_BOTTOMUP, -20, HEIGHT - 20, 20, this.buttonOpenTerminal, this.buttonOpenStorage, this.buttonCollectCoins).reposition();
    ...
    @Override
    protected void drawBackground(MatrixStack pose, float delta, int mouseX, int mouseY) {

        //Background
        RenderSystem.setShader(GameRenderer::getPositionTexShader);
        RenderSystem.setShaderTexture(0, GUI_TEXTURE);
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        this.drawTexture(pose, this.getGuiLeft(), this.getGuiTop(), 0, 0, this.getImageWidth(), this.getImageHeight());

        //Slot Machine
        if(this.isInteractMode())
            this.slotRenderer.render(pose, delta);

    }

    @Override
    protected void drawForeground(MatrixStack pose, int mouseX, int mouseY) {
        this.textRenderer.draw(pose, this.playerInventoryTitle, 8, this.getImageHeight() - 94, 0x404040);

        String valueText = MoneyUtil.getStringOfValue(this.handler.getContext(null).getAvailableFunds());
        this.textRenderer.draw(pose, valueText, 170 - this.textRenderer.getWidth(valueText) - 10, this.getImageHeight() - 94, 0x404040);
    }
    ...
    public void render(MatrixStack pose, int mouseX, int mouseY, float delta) {
        ...
        if(this.buttonInteract != null && this.buttonInteract.isMouseOver(mouseX, mouseY))
            this.renderTooltip(pose, this.getInteractionTooltip(1), mouseX, mouseY);
        if(this.buttonInteract5 != null && this.buttonInteract5.isMouseOver(mouseX, mouseY))
            this.renderTooltip(pose, this.getInteractionTooltip(5), mouseX, mouseY);
        if(this.buttonInteract10 != null && this.buttonInteract10.isMouseOver(mouseX, mouseY))
            this.renderTooltip(pose, this.getInteractionTooltip(10), mouseX, mouseY);
        if(this.buttonInfo != null && this.buttonInfo.isMouseOver(mouseX, mouseY))
            this.renderTooltip(pose, this.getInfoTooltip(), mouseX, mouseY);

        IconAndButtonUtil.renderButtonTooltips(pose, mouseX, mouseY, this.children());

        for(SlotMachineEntryDisplayWidget w : this.infoWidgets)
            w.renderTooltips(pose, mouseX, mouseY);
