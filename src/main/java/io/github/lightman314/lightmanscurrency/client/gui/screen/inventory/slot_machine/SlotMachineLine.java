...
public final class SlotMachineLine {
    ...
    public void render(MatrixStack pose, float partialTicks, int x, int y)
    {
        //Limit partial tick to be <= 1 so that the items don't clip beyond the overlay
        float partialTick = MathUtil.clamp(partialTicks, 0f, 1f);
        if(this.lockDelay != 0)
            y += (int)((float)BLOCK_SIZE * partialTick);
        this.previousBlock2.render(pose, ++x, ++y);
        this.previousBlock1.render(pose, x, y + BLOCK_SIZE);
        this.centerBlock.render(pose, x, y + (2 * BLOCK_SIZE));
        this.nextBlock.render(pose, x, y + (3 * BLOCK_SIZE));
    }
    ...
    private void playDing()
    {
        ...
        if(soundManager != null)
            soundManager.play(PositionedSoundInstance.master(SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, 1f));
    }
